...
PhysicalCamera::PhysicalCamera(const Camera& camera, float focalLength, QImage image) :
	Renderable(QMatrix4x4()),
...
void PhysicalCamera::paint(QOpenGLContext* context, const Camera& camera)
{
	...
	if (m_imageProgram && m_imageNumberVertices > 0)
	{
		...
		f->glDrawArrays(GL_TRIANGLES, 0, m_imageNumberVertices);
	...
	if (m_frustumProgram && m_frustumNumberVertices > 0)
	{
		...
		f->glDrawArrays(GL_LINES, 0, m_frustumNumberVertices);
